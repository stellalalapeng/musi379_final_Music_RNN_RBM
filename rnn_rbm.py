...
def rnnrbm():
    ...
    def generate(num, x=x, size_bt=size_bt, u0=u0, n_visible=n_visible, prime_length=100):
        ...
        [_, _, _, _, _, music] = control_flow_ops.while_loop(lambda count, num_iter, *args: count < num_iter,
                                                         ...
                                                         tf.zeros([1, n_visible],  tf.float32)], #loop_vars
                                                             shape_invariants=[tf.constant(1, tf.int32).get_shape(),
                                                                               tf.constant(num).get_shape(),
                                                                               U.get_shape(),
                                                                               tf.zeros([1, n_visible], tf.float32).get_shape(),
                                                                               x.get_shape(),
        ...
        # [_, _, _, _, _, music] = tf.while_loop(
        #     lambda count, num_iter, *args: count < num_iter, generate_recurrence,
        #     loop_vars,
        #     shape_invariants=[time_steps.get_shape(), iterations.get_shape(), U.get_shape(), u_t.get_shape(),
